 ...
 * \file HsExtCommon.h
 *
 * Common defines shared between HsExt.h (#include for 68K programs)
...
typedef enum 
  {	/*
	 ...
	 */
	hsAttr68KLidOpen,				// UInt16  - see below: hsAttrLidOpen
	hsAttr68KSysSleepReason,		// UInt16  - see below: hsAttrSysSleepReason
	hsAttr68KDisplayOn,				// UInt16  - see below: hsAttrDisplayOn
	hsAttr68KRingSwitch,			// UInt16  - see below: hsAttrRingSwitch
	hsAttr68KPostProcessRsc,		// UInt32  - see below: hsAttrPostProcessRsc
	hsAttr68KSysActive,				// UInt16  - see below: hsAttrSysActive
	hsAttr68KActiveCalls,			// UInt16  - see below: hsAttrActiveCalls
	hsAttr68KPhoneType,				// UInt32  - see below: hsAttrPhoneType
	hsAttr68KKeyboardLocked,		// Boolean - see below: hsAttrKeyboardLocked
	hsAttr68KKeyLockNextWakeTicks,	// UInt32  - see below: hsAttrKeyLockNextWakeTicks
  	
	/*
	 * All new 68K and ARM code shall use only the following 
	 * attributes, all of which take a 32-bit value parameter.
	 */
	hsAttrLidOpen = 0x80,		/**< false=closed, true=open */
	hsAttrSysSleepReason,		/**< Same as the SleepEventParamType.reason
						    		   ...
						    		   (e.g. hsSysSleepLid) */
	hsAttrDisplayOn,			/**< true= lid open, false=lid closed
						    		   (returns hsErrNotSupported if no lid) */
	hsAttrRingSwitch,			/**< State of ring switch */
	hsAttrPostProcessRsc,		/**< Pointer to resource containing the post-
						        		   processing lists */
	hsAttrSysActive,			/**< Bits representing various system activity */
	hsAttrActiveCalls,			/**< Number of active voice calls */
	hsAttrPhoneType,			/**< CDMA or GSM phone */
	hsAttrKeyboardLocked,		/**< true = keyguard active */
	hsAttrKeyLockNextWakeTicks,	/**< depricated */
	hsAttrKeyboardDB,			/**< DmOpenRef of database containing
	...
	hsAttrHotSyncSuppress,		/**< count of requests to suppress HotSync
										Set to non-zero to increment, set to zero to decrement */
	hsAttrAutoSnoozeAttns		/**< true = auto snooze the current instance of the attn dialog */
	/*
...
typedef enum
  {			
	...
	hsPrefNetworkTimeSelected,		  /**< UInt16: used by CDMA devices - should it read
	...
	hsPrefTouchscreenDisableFlags,	  /**< UInt16: Used by the phone app to determine when
													to disable touchscreen */

	// The following are used in SysHandleEvent() to launch apps in response
	//  to associated virtual characters.
    hsPrefOptHardPowerCharAppCreator, /**< UInt32 : Creator type of app to launch
									  		  in response to hsChrOptHardPower */

    hsPrefOptReservedCharAppCreator,  /**< UInt32 : Creator type of app to launch
									  		  in response to hsChrOptReserved (for future use) */
...
typedef enum
  {
	...
	hsLightCircumstanceAlertWokeDevice	= 0,	// An important alert woke the
												//  device up from sleep.
	hsLightCircumstanceQuickTyping		= 1,	// We want the user to be able
												//  to access the keyboard easily
	hsLightCircumstanceUserNotLooking	= 2,	// We suspect the user isn't
	...
	hsLightCircumstanceNumCircumstances,		// Num circumstances.  
												// SUBJECT TO CHANGE.
...
typedef enum
  {
	kIndicatorCountGetState	= 0,
	kIndicatorCountForever	= 0xFFFF
  }
...
#define hsHALIDHandspringOs35_40	  'Hs01'	// Used by all Handspring OS 3.5 HALs
												// and 4.0 HALs
...
#define hsDeviceIDOs5Device1	  'H101'	// Handspring OS5 device #1
#define hsDeviceIDOs5Device1Sim	  'H201'	// Handspring OS5 device #1 Simulator
