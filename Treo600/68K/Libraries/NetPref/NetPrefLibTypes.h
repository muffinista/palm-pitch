...
enum
  {
	...
	netPrefSvcMediumOEM		= 9,	// This service is a place-holder for an
									// external OEM plugin
...
enum
  {
	// Never time out due to data idleness
	netPrefNetIdleTimeoutNever			= (UInt16) 0
  };
...
enum
  {
	// Basic algrorithm: each time: try primary; if fails, try fallback;
...
enum
  {
	// Indicates that the last login status of the
	// corresponding network service is unknown.
	netPrefMutexStickyFBAStatusUnknown		= 0,

	
	// Indicates that the last login attempt using the
	// corresponding network service failed.
	netPrefMutexStickyFBAStatusFailed		= 1,

	// Indicates that the last login attempt using the
	// corresponding network service succeeded.
	netPrefMutexStickyFBAStatusSucceeded	= 2
